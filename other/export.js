var Export = {
    ...
    getMarkerRect: {
        getTriRect: function(i, d, j, c) {
            var b, f, e, g, a;
            b = Math.PI / 2 - i;
            outer_t = c / Math.tan(b / 2), e = c / Math.sin(i);
            g = d + outer_t * 2, a = j + c + e;
            return {
                x: -c,
                y: -outer_t,
                w: a,
        ...
        getArrowRect: function(i, d, j, c) {
            var b, f, e, g, a;
            f = c * Math.sin(i), outer_t = c * Math.cos(i), e = c / Math.sin(i);
            g = d + outer_t * 2, a = j + f + e;
            return {
                x: -f,
                y: -outer_t,
                w: a,
        ...
        getDiagRect: function(b, e, d) {
            var i, g, a, c, f;
            i = b * b - e * e, i = Math.sqrt(i), a = d * i / b, g = d * e / b;
            outerH = e * 2 + a * 2, c = i + g * 2;
            return {
                x: -g,
                y: -a,
                w: c,
...
Export.svg = {
    draw2svg: function(J, aq, N, S, B, r) {
        ...
        if (J.siblings(".tp_expand_box")[0]) {
            ...
            if (z == "underline") {
                Z = an - 7, R = an + Math.ceil(G / 2)
            } else {
                Z = an / 2 - 7, R = an / 2 + Math.ceil(G / 2)
            }
            if (j < 0) {
                ai = -15, U = 0, ar = -l.width()
            } else {
                ai = W + 1, U = W, ar = l.width()
            }
            c = [
                ["M", U, R],
                ["l", ar, 0]
        ...
        if (z == "round") {
            aa = [
                ["M", 0, an / 2],
                ["A", W / 2, an / 2, 0, 0, 1, W / 2, 0],
                ["A", W / 2, an / 2, 0, 1, 1, 0, an / 2],
                ["Z"]
            ];
            ac = ao.path(aa)
        } else {
    ...
    line2svg: function(e, v, D) {
        ...
        var C = [
            ["M", y(n.x), y(n.y)],
            ["C", y(l.x), y(l.y), y(j.x), y(j.y), y(m.x), y(m.y)]
        ];
        var M = e.group();
    ...
    createIcon: function(j, n, t, u) {
        ...
        var r = [
            ["M", 0, p / 2],
            ["a", p / 2, p / 2, 0, 0, 1, p / 2, -p / 2],
            ["a", p / 2, p / 2, 0, 1, 1, -p / 2, p / 2]
        ];
        if (n == "link" || n == "note") {
            b.attr({
    ...
    shape2svg: function(b, t, d) {
        ...
        for (var I = 0; I < z.length; I++) {
            F = z[I], u = F.line, H = F.fill, E = "#fff", f = u.lineWidth, D = {
                solid: ["none"],
                dashed: [f * 5, f * 2],
                dot: [f, f * 1.5],
    ...
    flowline2svg: function(b, l, q) {
        ...
        if (w.text.length) {
            a = this.getRect(y), b.w = a.width, b.h = a.height, b.x = a.left, b.y = a.top;
            var n = u.textAlign,
                t = "Microsoft Yahei",
                j = u.size,
                d = u.bold,
    ...
    json2path: function(props, paths, line, fill) {
        var path, pathArr = [],
            lineStyle, fillStyle, l, f, actions, action, hex, c1, c2, r, g, b, lineWidth, w = props.w,
            h = props.h,
            x = props.x,
            y = props.y;
        for (var i = 0; i < paths.length; i++) {
            path = [], l = $.extend({}, line), f = $.extend({}, fill), lineStyle = $.extend({}, paths[i].lineStyle), fillStyle = $.extend({}, paths[i].fillStyle), actions = paths[i].actions;
            if (fillStyle && fillStyle.type == "none") {
                ...
                if (fillStyle && /^r-/.test(fillStyle.color)) {
                    ...
                    c1 = hex.replace(/rgb|\(|\)|\s/gi, "").split(","), c2 = fillStyle.color.replace(/\s/g, "").split(","), r = c1[0], g = c1[1], b = c1[2];
    ...
    line2path: function(f) {
        ...
        p = [
            ["M", h.x, h.y]
        ].concat(this.toPath[c](f.points, j)), k.mid = this.getPostOnLine[c](h, f.points, j, 0.5);
        return [{
            actions: p,
            text: f.text.split("\n")
        }, k]
    ...
    marker2svg: function(j, l, g) {
        var a = [],
            ...
            c = j.markerOffset,
            k = 4,
            f = Schema.config.markerSize,
            p = b.length * f + (b.length - 1) * k,
            n = l.w / 2 - p / 2,
            m = l.h - f - c,
            r = {
                lineColor: "50,50,50",
        ...
        for (var d = 0; d < b.length; d++) {
            o = {}, e = Schema.markers[b[d]].call(g, f), o.h = f, o.w = p, o.x = n, o.y = m, o.line = r, o.line.lineColor = Export.getColor(o.line.lineColor);
            o.fill = q;
            if (o.fill.alpha != 1) {
    ...
    attr2svg: function(g, m, b) {
        ...
        for (var c = 0; c < m.length; c++) {
            ...
            if ((d.value || d.icon) && d.showType && d.showType != "none") {
                f.text = "", f.name = d.name, f.value = d.value, f.showType = d.showType, f.type = d.type, a = $("#attr_canvas_" + d.id);
                if (d.showName) {
                    f.text = d.name + "："
                }
                if (d.icon) {
    ...
    getMarkerArrow: function(s, o, b) {
        var a, k, e, p, j, g = b ? 180 : 0,
            q, c = s.lineWidth,
            f = c / 2,
            i = Math.PI,
            n = Export.getMarkerRect,
            m;
        return {
            normal: function() {
                var d = 12,
            ...
            solidArrow: function() {
                p = 12, arrowAngle = Math.PI / 10, j = p * Math.tan(arrowAngle);
            ...
            solidDiamond: function() {
                p = 8, arrowAngle = Math.PI / 7, j = p / Math.tan(Math.PI / 2 - arrowAngle);
                m = n.getTriRect(arrowAngle, j * 2, p, f);
                m.x = -m.r, m.w = m.w * 2 - c;
                if (!o) {
                    return m
            ...
            cross: function() {
                e = 14, j = 12, m = n.getDiagRect(e, j / 2, f);
                if (!o) {
                    return m
                }
                return o.marker(m.w, m.h, function(d) {
                                ...
                                var pattern = draw.pattern(shape.w, shape.h, function(add) {
                                    ...
                                    if (fill.display == "fill") {
                                        if (shape.w / shape.h > imgW / imgH) {
                                            imgH = imgH * shape.w / imgW, imgW = shape.w
                                        } else {
                                            imgW = imgW * shape.h / imgH, imgH = shape.h
                                        }
                                    }
    ...
    textSplit: function(props, blocks, font) {
        var h, w, tw, div_temp, block, text = {},
            post, f, width, val, lines, line, textTemp, lineBlock, lineBlocks = [],
            name = font.orientation;
        h = w = /horizontal/.test(name) ? props.h : props.w;
        for (var k = 0; k < blocks.length; k++) {
            block = blocks[k], f = $.extend(block.font, $.extend({}, font));
            post = block.position;
            ...
            h = props.h, w = props.w, f = $.extend(f, block.fontStyle);
    ...
    getPostOnLine: {
        curve: function(h, c, g, b) {
            var a, f, e = c[0],
                d = c[1];
            a = h.x * Math.pow((1 - b), 3) + 3 * e.x * b * (1 - b) * (1 - b) + 3 * d.x * b * b * (1 - b) + g.x * b * b * b, f = h.y * Math.pow((1 - b), 3) + 3 * e.y * b * (1 - b) * (1 - b) + 3 * d.y * b * b * (1 - b) + g.y * b * b * b;
            return {
                x: a,
                y: f
            }
        ...
        broken: function(h, m, j, n) {
            ...
            a = k(h, m[0]), c += a.len, o[0] = a;
            for (var b = 0; b < m.length; b++) {
                if (b == m.length - 1) {
                    a = k(m[b], j), c += a.len, o.push(a)
                } else {
                    a = k(m[b], m[b + 1]), c += a.len, o.push(a)
                }
            }
            a = 0;
        ...
        normal: function(f, c, e, b) {
            var a, d;
            a = f.x + Math.abs((f.x - e.x) * b), d = f.y + Math.abs((f.y - e.y) * b);
            return {
                x: a,
                y: d
            }
